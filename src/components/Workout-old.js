...
//render the exercises from the single workout passed to this component
export class Workout extends Component {
  ...
  handleSearch = (event) => {
    ...
    fetch(
      `https://www.googleapis.com/books/v1/volumes?q=${this.state.search}&key=AIzaSyAOOYu0LFVQ45qOFjMl11H8Mdr2NYDxLqU`
    )
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
  ...
  render() {
    const existingBooks = this.state.books.map((book, key) => {
    ...
    const apiResults = this.state.booksFound.map((book, key) => {
      return (
        <div key={key} className="apiResultsItem">
          <form onSubmit={this.addBookFromApi} id="bookSearch">
            ...
            <input
              type="hidden"
              defaultValue={this.props.match.params.collectionId}
              name="collection_id"
            ></input>
    ...
    return (
      <div className="App">
        ...
        <main className="addmain">
          ...
          <div id="apiResults">{apiResults}</div>
        </main>
      </div>
    );
